...
- Visibility (#40): Place + Memory have `isPublic` (default true). Private = local-only, never uploaded; saving a private item deletes any server copy. Shared `VisibilityToggle` widget (Public / Just me)
- Type change (#41): clue detail has an Edit sheet (type chips + visibility); `TraceShapeRecordingPage` takes `TraceShapeArgs` (center/title/onSave) so Places and 'place'-type clues share the walk/draw tracing flow; `memories.boundary` JSONB; memories upsert is DO UPDATE
- Accounts (#42, optional): Google SSO (ID token verified server-side via Google tokeninfo; button hidden until `kGoogleServerClientId` set — see docs/GOOGLE_SIGNIN_SETUP.md) + email/password (bcrypt); opaque UUID bearer tokens in `auth_tokens`; `user_id` nullable on places/memories — anonymous uploads still work; session persisted in Documents/clue_auth.json
- Community (#43): 5th nav tab "Hive" — GET /api/community/top leaderboard (top 20 by public places+clues, anonymous count separate). User explicitly requested this social feature (overrides the earlier no-social-features rule for this scope)
